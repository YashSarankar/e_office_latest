...
class _Widget2State extends State<Widget2> {
  ...
  @override
  Widget build(BuildContext context) {
    final List<String> salarySeries = salaryData['salarySeries'];
...
class _Widget3State extends State<Widget3> {
  ...
  Future<void> _fetchData() async {
    try {
      ...
      final data = await _apiService.getReceiptMonthlyCount(userId);
  ...
  Future<void> getReceiptData() async {
    try {
      SharedPreferences prefs = await SharedPreferences.getInstance();
      String? userId = prefs.getString('id');
      getReceiptResponse = await _apiService.getReceipt(userId!);
    } catch (e) {
      setState(() {
        error = e.toString();
      });
