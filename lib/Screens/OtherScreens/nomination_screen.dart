...
import 'package:e_office/Screens/OtherScreens/nomination_form.dart';
...
Future<void> _fetchNomineeData() async {
  ...
  try {
    final response = await http.get(Uri.parse('https://eofficess.com/api/nominations/$userId'));
    ...
    builder: (context) {
      return AlertDialog(
        content: Column(
          ...
          children: [
            Text('Name: $name', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
            ...
            Column(
              ...
              children: [
                ElevatedButton(
                  ...
                  onPressed: () async {
                    final downloadUrl = 'https://eofficess.com/api/user-nomination-pdf/$id';
...
Future<void> _shareFile(BuildContext context, String filePath, String name) async {
  try {
...
// ----------------------------------------------------------------x
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Nomination Screen', style: TextStyle(color: Colors.white, fontSize: 20)),
        leading: IconButton(
                  ...
                  for (var item in _filteredData) // Create rows dynamically
                    TableRow(
                      children: [
                        ...
                        Container(
                            height: 60,
                            child: Center(child: Text("${item['nomination_type']}"))),
                        Container(
  height: 60,
  child: Center(
    child: Text(
      ...
      style: TextStyle(
        ...
        fontWeight: FontWeight.bold,
                        ...
                        Container(height: 60, child:IconButton(onPressed:()=>
                            _showAction(
                                item['nomination_type']!,item['status']!,item['sn']!),
