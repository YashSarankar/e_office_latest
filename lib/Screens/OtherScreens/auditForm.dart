...
import 'package:e_office/Screens/OtherScreens/audit_add_form.dart';
...
// To handle errors
  ...
  Future<void> _fetchAuditData() async {
    ...
    try {
      final response = await http.post(Uri.parse('https://eofficess.com/api/get-user-audit?user_id=$userId'));
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['success'] == true) {
          final List<dynamic> audits = responseData['data']; // Get the list of audits
          setState(() {
            // Convert dynamic response to a list of maps with string types
            _auditData = audits.map((audit) {
              return {
                'sn': audit['id']?.toString() ?? '', // Convert to string and handle null
                'audit_name': audit['audit_name']?.toString() ?? '', // Convert to string and handle null
                'status': audit['status']?.toString() ?? '', // Convert to string and handle null
              };
            }).toList();
    ...
    showDialog(context: context, builder: (context){
      return AlertDialog(
        content: Column(
          ...
          children: [
            Text('Name: $name', style: TextStyle(fontSize: 16,fontWeight: FontWeight.w600)),
            SizedBox(height: 8),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Audit Data', style: TextStyle(color: Colors.white, fontSize: 20)),
        leading: IconButton(
          ...
          Expanded(
            child: SingleChildScrollView(
              child: Table(
                ...
                children: [
                  for (var item in _filteredData) // Create rows dynamically
                    TableRow(
                      children: [
                        Container(
                            height: 60,
                            child: Center(child: Text('${item['sn']}'))),
                        Container(
                            height: 60,
                            child: Center(child: Text("${item['audit_name']}"))),
                      Container(
  height: 60,
  child: Center(
    child: Text(
      ...
      style: TextStyle(
        color: item['status'] == 'approved'
            ...
            : item['status'] == 'rejected'
                ? Colors.red
                : Colors.orange, // For 'pending' or other statuses
        fontWeight: FontWeight.bold,
                        ...
                        Container(height: 60, child:IconButton(onPressed:()=>
                            _showAction(
                                item['audit_name']!,item['status']!,item['sn']!),
