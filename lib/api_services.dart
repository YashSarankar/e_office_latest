...
class ApiService {
  ...
  Future<List<String>> fetchChecklistNames() async {
    final response = await http.post(
      Uri.parse('https://eofficess.com/api/get-book-checklist'),
    );
  ...
  Future<Map<String, dynamic>> getSalary(int userId) async {
    final response = await http.post(
        Uri.parse('https://eofficess.com/api/get-salary?user_id=$userId'));
    if (response.statusCode == 200) {
      return json.decode(response.body);
    } else {
  ...
  Future<ReceiptByStatus> fetchReceiptsByStatus(int userId) async {
    final response = await http.post(
      Uri.parse('https://eofficess.com/api/getreceiptbystatus?user_id=$userId'),
    );
